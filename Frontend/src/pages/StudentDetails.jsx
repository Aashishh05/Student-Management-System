...
const StudentDetails = () => {
  ...
  const fetchStudent = async () => {
    try {
      setLoading(true);
      const res = await api.get(`/students/getStudent/${id}`);
          ...
          <div className="px-1 md:px-6 lg:px-10">
            <div className="flex flex-col md:flex-row items-start md:items-center justify-between p-8 md:p-10 rounded-2xl bg-slate-200 shadow-2xl border-l-[12px] border-b-[6px] border-[#2249A3] gap-6">
              <div className="flex flex-col font-serif">
                ...
                <p className="text-gray-500 mt-1 text-base md:text-lg">
                  Full profile information for this student
                </p>
              </div>
              <button
                onClick={() => nav("/Student")}
            ...
            {!student ? (
              ...
              <div className="mt-10 bg-slate-200 shadow-2xl  rounded-2xl hover:shadow-2xl hover:scale-105 transition-all duration-400 p-6 md:p-8 max-w-3xl mx-auto">
                ...
                <div className="grid grid-cols-1 sm:grid-cols-3 gap-4 mt-6 font-serif">
                  ...
                  <div className="border border-slate-300 rounded-xl p-4 bg-gradient-to-r from-purple-600 to-indigo-500">
                    <p className="text-l text-white mb-1">Teacher</p>
                    <p className="text-sm font-semibold text-white">
                      {student.teacher || "—"}
                    </p>
                  </div>
                  <div className="border border-slate-300 rounded-xl p-4 bg-gradient-to-r from-slate-700 to-blue-600">
                    <p className="text-l text-white mb-1">Payment Status</p>
                    <span
                      className={`inline-block px-3 py-1 text-xs font-bold rounded-full text-white shadow-md
  ${
